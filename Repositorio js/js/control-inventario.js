...
function calcularValorParte(parte) {
    ...
    if (parte == null){
        console.log("No se permiten null")
    } 

    var suma = parte.cantidad * parte.precio_unitario
    return suma

}

//Ejecicion de la funcion

partes.forEach(parte => {
    const mostrar = calcularValorParte(parte)
});
...
listado.forEach(function(parte) {
        Acumulador_Array += calcularValorParte(parte);  
    });
return Acumulador_Array
} 
...
function registrarEntrada(idParte, cantidad) {
    ...
    // Comprobacion para que no supere la cantidad maxima

    if (parte.cantidad + cantidad > parte.capacidad_maxima){
        return false
    }

    let entrada_valor = parte.cantidad + cantidad

    parte.cantidad = entrada_valor
...
function registrarSalida(idParte, cantidad) {
    ...
    let salida_cantidad = parte.cantidad - cantidad

    parte.cantidad = salida_cantidad
